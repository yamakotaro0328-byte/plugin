...
/** Left-click with the wand sets corner 1, right-click sets corner 2 (see /boss wand). */
public class SelectionListener implements Listener {
    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        if (event.getAction() == Action.LEFT_CLICK_BLOCK) {
            ...
            event.getPlayer().sendMessage(messages.get("boss.corner1-set", Map.of()));
        } else if (event.getAction() == Action.RIGHT_CLICK_BLOCK) {
            event.setCancelled(true);
            selectionManager.setCorner2(event.getPlayer().getUniqueId(), clicked.getLocation());
            event.getPlayer().sendMessage(messages.get("boss.corner2-set", Map.of()));
        }
    }
