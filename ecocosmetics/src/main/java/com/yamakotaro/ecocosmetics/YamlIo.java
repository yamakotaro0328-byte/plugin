 ...
 * ファイルを読み書きするため、そのデフォルトがUTF-8でない環境では設定・プレイヤーデータの
 * 非ASCII文字(日本語等)が読み込むたびに文字化けし、しかもその文字化けした内容がそのまま
 * 上書き保存されて元に戻せなくなる (EcoTPで実際に起きたバグ)。
 * plugin.getConfig()/reloadConfig() も内部で同じ問題のある読み込みをするため、このプラグインは
 * それらを使わず、常にこのクラス経由でconfig.ymlを読み書きする。
 */
