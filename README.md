...
`/roma` を実行すると、そのプレイヤー自身のチャットメッセージが、ローマ字入力から日本語
(かな/漢字混じり) へ自動変換されるようになります(もう一度実行するとオフに戻る、デフォルトは
オフ)。`config.yml` の `features.romaji-conversion` で機能自体を無効化できます。

既に日本語やURLを含むメッセージは変換対象になりません(ASCII文字だけのメッセージのみ)。
Google非公式の内部エンドポイント(`inputtools.google.com`、登録・APIキー不要)を利用して
おり、変換に失敗した場合は元のメッセージがそのまま送信されるだけです(チャット自体が
止まることはありません)。非公式のため、将来的に仕様変更・停止される可能性があります。

## 個人チャット (/msg、/reply)

`/msg <プレイヤー名> <メッセージ>` (エイリアス `/tell`・`/w`・`/pm`) で、他のプレイヤーには
見えない個人チャットを送れます。`/reply <メッセージ>` (エイリアス `/r`) で、直前に
...
`vote-reward.sites` に投票サイトの名前とURLを登録しておくと、`/menu` の投票タイルを
クリックしたときにクリックで開けるリンクとしてチャットに一覧表示されます (任意、未設定なら
説明表示のみ)。
