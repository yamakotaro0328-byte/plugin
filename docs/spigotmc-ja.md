...
[*][B]投票報酬(別プラグイン不要)[/B] — Votifier(V1)プロトコル互換のリスナーを内蔵しているため、投票サイトから直接この鯖に投票を送れます(RSA鍵ペアは自動生成)。既にNuVotifierを他プラグインのために使っている場合もそちらを自動検出します。いずれの方式でも、投票したプレイヤーには報酬(デフォルト1000円、設定変更可)とサーバー全体への通知が付与されます。投票時にオフラインだった場合は保留され、次回ログイン時に付与されます。
[/LIST]
...
[*]投票報酬は内蔵リスナーで単体動作 — NuVotifierが必要なのは、既存のNuVotifier環境をそのまま使いたい場合のみです
[/LIST]
