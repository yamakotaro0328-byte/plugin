 ...
 * 置換する。plugin.config() (EcoPotionsPlugin が独自にUTF-8で読み込んだ設定) を経由するため、
...
public class Messages {
    ...
    public Messages(EcoPotionsPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    /** effects.<type> (例: "haste" -> "採掘速度上昇") の表示名。未登録ならtypeをそのまま返す。 */
    public String effectName(String type) {
        String fullPath = "messages." + language() + ".effects." + type.toLowerCase();
        return plugin.config().getString(fullPath, type);
    }
}
