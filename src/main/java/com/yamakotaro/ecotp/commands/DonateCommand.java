...
/**
 * /donate <プレイヤー名> <金額> : /pay と同じ送金処理だが、成功したらサーバー全体に
 * 寄付を知らせる(相手が /donatemessage で設定したお礼メッセージ、無ければデフォルト)。
 */
public class DonateCommand implements CommandExecutor, TabCompleter {

    private final EcoTpPlugin plugin;

    public DonateCommand(EcoTpPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 2) {
            player.sendMessage(plugin.msg("donate.usage"));
            return true;
        }
        ...
        String actionKey = "donate:" + target.getName() + ":" + args[1];
        ...
        String description = plugin.getMessages().get("donate.description", "player", targetName);
        plugin.getConfirmationManager().request(player, actionKey, amount, description, () -> {
            ...
            player.sendMessage(plugin.msg("donate.sent", "player", targetName, "amount", ChatUtil.formatMoney(amount)));
            plugin.getDonationManager().broadcastDonation(player, currentTarget, amount);
        });
        return true;
    }
