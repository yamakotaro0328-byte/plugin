...
[*][B]完全GUI駆動のアドミンショップ[/B] — [I]/adminshop[/I]でチェストGUIを開き、プレイヤーは左クリックで購入・右クリックで売却(シフトクリックでスタック/所持している全個数)できます。[I]/adminshop admin[/I]で編集モードを開き、アイテムを手に持って空きスロットをクリックすると出品、出品済みのアイテムを左クリックで購入価格、右クリックで売却価格を設定(金床のリネーム欄に数値を入力するだけで、コマンドは一切不要)、シフトクリックで削除できます。在庫管理は無く、一般的なアドミンショップと同様に常に無限の在庫・資金として扱われます。
