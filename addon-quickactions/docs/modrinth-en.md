...
- **Player-to-player shop** — `/pshop browse` (alias `/ps`) shows every player's active listings in one GUI; left-click buys 1, shift-left buys the whole remaining stock, and payment goes straight from buyer to seller. `/pshop my` manages your own listings: hold an item and click an empty slot, then set the quantity and price per item with +/- buttons in a GUI; click a listing to take it down and get your items back. Unlike the admin shop, stock is real and limited to what sellers actually listed. `/pshop history` shows your own last trades (as buyer and seller).
